...
uint32_t floatColorToUInt(const float* color) {
	float clampedColor[4];
	for (int i = 0; i < 4; ++i) {
		clampedColor[i] = max(0.0f, min(1.0f, color[i]));
	}
	unsigned char r = static_cast<unsigned char>(clampedColor[0] * 255.0f);
	unsigned char g = static_cast<unsigned char>(clampedColor[1] * 255.0f);
	unsigned char b = static_cast<unsigned char>(clampedColor[2] * 255.0f);
	unsigned char a = static_cast<unsigned char>(clampedColor[3] * 255.0f);
	return static_cast<uint32_t>(a) << 8 | static_cast<uint32_t>(b) << 16 | static_cast<uint32_t>(g) << 24 | static_cast<uint32_t>(r);
}
...
void R_AddCmdDrawStretchPic_Detour(float* x, float* w, const float* col, material_s* shader)
{
